...
export default function StatCard({ title, value, icon: Icon, trend, trendType = "up", iconBgClass }) {
  return (
    <div className="bg-white/80 backdrop-blur-md border border-slate-200/50 p-6 rounded-2xl shadow-sm hover:-translate-y-0.5 transition-all duration-300">
      <div className="flex justify-between items-start mb-4">
        {/* এখানে Icon সরাসরি রেন্ডার হবে */}
        <div className={`p-3 rounded-xl ${iconBgClass || "bg-slate-100 text-[#040d1b]"}`}>
      ...
      <h3 className="text-2xl font-bold text-[#040d1b] mt-1">{value}</h3>
    </div>
  );
}